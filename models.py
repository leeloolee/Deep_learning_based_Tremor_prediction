...
from tensorflow.keras.layers import Flatten, Conv1D, Input, MaxPool1D, GlobalAveragePooling1D, Dropout, Dense, \
    Bidirectional, GRU, TimeDistributed, Lambda, Concatenate, Reshape, Subtract, Add, Activation, BatchNormalization, LSTM, SimpleRNN
...
def LSTM_model():
    ...
    model.add(LSTM(20, input_shape=(120,1))) # (timestep, feature)
...
def RNN_model():
    model = Sequential()  # Sequeatial Model
    model.add(SimpleRNN(20, input_shape=(120,1))) # (timestep, feature)
    model.add(Dense(1))  # output = 1
    model.compile(loss='mse', optimizer='Adam', metrics=['mse'])
    return model
